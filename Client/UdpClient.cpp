...
int32_t CUdpClient::sendMsg(std::string_view message) {
    ...
    //messagePacket.msg_name = res->ai_addr;
    //messagePacket.msg_namelen = res->ai_addrlen;
    messagePacket.msg_iov = iov;
    messagePacket.msg_iovlen = 1;
    messagePacket.msg_control = 0;
    messagePacket.msg_controllen = 0;

    //if (::sendmsg(m_Socket, &messagePacket, 0) == -1) {
...
int32_t CUdpClient::getWaitTimeMs(int32_t failCount) const {
	...
	/*
      Original Reference: wait_interval = (base * 2^n) +/- (random_interval) 
	  Now: wait_interval = (base * multiplier^n) * (1 - random_interval_ratio[0.0 ~ 0.2))	  
	*/
